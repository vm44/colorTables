...
class ColorDialog extends Component{
  ...
  onOK=()=>{
    if(this.state.colorChanged)this.addColor(this.state.cColor)
  ...
  applyColor=(color)=>{
    if(this.state.edColor){
      this.props.dispatch({type:'chColorImm',val:{key:this.props.colorEntry[0],prevColor:this.state.edColor,color:color}})
      this.setState({ edColor: color })
